<header class="site-header py-3 shadow" id="header">
  <nav class="d-flex  justify-content-between justify-content-center menu-desktop-baterrey">
      <a class="ms-md-5" href="index" aria-label="Product">
        <div class="logo-baterrey"></div>
      </a>
      <div class="d-flex justify-content-end align-items-center me-5">
        <a class="py-2 text-dark mx-3" href="cart">
          <div class="d-flex justify-content-center align-items-center mb-2">
            <div class="position-absolute fs-5 mt-3 fw-bold" space="num_cart"></div>
                <svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" fill="currentColor" class="bi bi-bag" viewBox="0 0 16 16">
                  <path d="M8 1a2.5 2.5 0 0 1 2.5 2.5V4h-5v-.5A2.5 2.5 0 0 1 8 1zm3.5 3v-.5a3.5 3.5 0 1 0-7 0V4H1v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V4h-3.5zM2 5h12v9a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V5z"/>
                </svg>
          </div>
        </a>
      </div>
  </nav>
</header>